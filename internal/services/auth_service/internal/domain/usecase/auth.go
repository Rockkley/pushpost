...
import (
	...
	"github.com/rockkley/pushpost/internal/services/auth_service/internal/repository"
...
func (s *AuthUsecase) Register(ctx context.Context, data dto.RegisterUserDto) (*dto.RegisterResponseDto, error) {
	...
	regResponse := &dto.RegisterResponseDto{
...
func (s *AuthUsecase) Login(ctx context.Context, dto dto.LoginUserDTO) (string, error) {

	user, err := s.userClient.AuthenticateUser(ctx, dto.Email, dto.Password) // fix me
	if err != nil {
		return "", err
	}
